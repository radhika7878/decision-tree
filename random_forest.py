...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #
        # DONE: Create a sample dataset of size n by sampling with replacement
        ...
        rand = np.random.choice(len(XX), n)
        for i in range(n):
            samples.append(XX[rand[i]][:-1])
            labels.append(XX[rand[i]][-1])
    ...
    def fitting(self):
        # DONE: Train `num_trees` decision trees using the bootstraps datasets
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #  for any of the trees. : assigned random 0 or 1 value
...
def main():
    ...
    # Loading data set
    print("reading data")
    with open("BankMarketing.csv") as f:
        next(f, None)
    ...
    # DONE: Initialize according to your implementation
